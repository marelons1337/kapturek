...
RSpec.describe(Property::Payment, type: :model) do
  describe ".kind_model_values" do
    it "returns an array of arrays with the name and id of the income" do
      income = create(:property_income)
      expect(Property::Payment.kind_model_values("income")).to eq(Property::Income.all.each.map { |i| [i.name, i.id] })
    end

    it "returns an array of arrays with the name and id of the expense" do
      expense = create(:property_expense)
      expect(Property::Payment.kind_model_values("expense")).to eq(Property::Expense.all.each.map { |e| [e.name, e.id] })
    end
  end
